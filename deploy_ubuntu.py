...
def main():
    ...
    run(ssh, "apt-get install -y -qq python3 python3-pip python3-venv curl", timeout=180)
    out, _, _ = run(ssh, "python3 --version")
    print(f"  Python: {out}")

    # 2. Загрузка кода
    print("\n[2] Загрузка кода...")
    core_files = ["bot.py", "db.py", "config.py", "api.py", "price_data.py", "requirements.txt", "price.json"]
...
"""
    ...
    for svc in SERVICES:
        ...
        if not crm_ok or not bot_ok:
            out_err, _, _ = run(ssh, f"journalctl -u {svc_id}-crm --since 2min --no-pager 2>&1 | grep -i -E 'error|traceback|import' | tail -5")
            if out_err:
                print(f"    Ошибки: {out_err[:300]}")

    ssh.close()
    print("\n" + "=" * 60)
    print("  ДЕПЛОЙ ЗАВЕРШЁН")
    print("=" * 60)
    print(f"\n  kaskad: http://{NEW_SERVER}:8000/login")
    print(f"  hairos: http://{NEW_SERVER}:8005/login")
